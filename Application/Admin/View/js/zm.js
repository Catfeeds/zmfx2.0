...
$(window).resize(function(){
	$('.menu,.index-right,.index-left').height($(window).height()-69);
	Cwidth = $(window).width()-161;//主体框宽度		
	$('.content').width(Cwidth);
	$('.content').css('min-height',$(window).height());//主体框高度
...
$(document).ready(function(){
	...
	$('#nav_type').change(function(){
		...
		if(id == 1){
			...
			$('#nav_url input[name=nav_url]').val('/Home/Goods/goodsSeries/gsid/'+$('select[name=goods_series_id]').val()+'.html');//默认第一个
		}
	})
	...
	$('.addCaigouOrder').click(function(){
		...
		var og = '';
		var og1 = '';		
		$("input[name='add_caigou[]']").each(function(){
			if($(this).prop('checked'))
			{
				...
				if (og1 == '') {
					if($("input[name='beizhu_"+$(this).val()+"']").val() !='' &&  $("input[name='beizhu_"+$(this).val()+"']").val() != undefined && $("input[name='beizhu_"+$(this).val()+"']").val() !=null){
						og1 =  $("input[name='beizhu_"+$(this).val()+"']").val();
					}else{
						og1 =  '0';
					}					
				} else {
					if($("input[name='beizhu_"+$(this).val()+"']").val() !='' &&  $("input[name='beizhu_"+$(this).val()+"']").val() != undefined && $("input[name='beizhu_"+$(this).val()+"']").val() !=null){
						og1 += '___' + $("input[name='beizhu_"+$(this).val()+"']").val();
					}else{
						og1 += '___' +'0';
					}
		...
		par   += og + ','; 									 //订单order_goods的产品id，做个有用，多个分号隔开
		par   += '1';     									 //$is_caigoudan，做个有用
		par   += new Date().getTime();     				 	//$is_caigoudan，做个有用
 								
		document.cookie="beizhu="+ og1+";path=/";			//备注	
		
		_this  = this;
		$.get("/Admin/Order/OrderAddOrder",{par:par},function(data){
			if(data.success == 'true'){
				alert(data.msg);
			}else{
				alert(data.msg);
		...
		// document.cookie="AddCaigouOrderNameTime="+ new Date().getTime();
		return false;
	});
