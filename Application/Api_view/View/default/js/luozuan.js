...
/*提交数据执行查询*/
function submitData(){
	_ajax(getCookie('diamondsDataUrl'),'t='+Math.random()+'&type='+request('type')+'&uid='+get_uid+'&token='+get_token+'&agent_id='+get_agent_id, 'GET', diamondsData_callback, true);
}
/*处理程序返回的JSON查询数据*/
...
function submitDataNextpage(){
	_ajax(getCookie('diamondsDataUrl'),'t='+Math.random()+'&type='+request('type')+'&uid='+get_uid+'&token='+get_token+'&agent_id='+get_agent_id, 'GET',diamondsData_callbackNextpage, true);
}
