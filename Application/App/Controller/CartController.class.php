...
class CartController extends AppController{
	...
	**/
	public function getCartGoods($goods_id){
		...
		if($_SESSION['app']['uid'] or $_COOKIE['PHPSESSID']) {
			if(!empty($goods_id)){
				$cartGoods = M('cart')->where("agent_id=".C('agent_id')." AND uid=".$_SESSION['app']['uid']." and id in ($goods_id)")->select(); 
			}else{
				...
				switch($val['goods_type']){
					case 1: // 1 表示裸钻
						if($goods_id){
							...
							$cartGoods[$key]['goods_attrs'] = getGoodsListPrice($cartGoods[$key]['goods_attrs'], $_SESSION['app']['uid'], 'luozuan',  'single');				
					...
					case 2:// 散货
						...
						break;
					case 3:     // 成品
						/* $goods_id		= $cartGoods[$key]['goods_id'];
						$sku_sn				= $cartGoods[$key]['goods_attrs']['goods_sku']['sku_sn'];
						$temp				= $Goods->getGoodsSku($goods_id," sku_sn = '$sku_sn' "); 
						$temp['goods_type']	= 3;
					...
					case 4:// 戒托  
						...
						foreach($pp_luozuan as $k=>$v){
							...
							//$cartGoods[$key]['pp_luozuan']['stopgap_id']=max($stopgap_id);
						//如果为活动商品则取值活动价格，否则取值普通售卖价格
						if(in_array($cartGoods[$key]['goods_attrs']['activity_status'],array('0','1'))){
	...
	**/
	public function cartMatch(){
		...
		$data['data']         = getGoodsListPrice($data['data'], $_SESSION['app']['uid'], 'luozuan');
	...
	**/
	public function  Confirm_Cartapi(){
		...
		if(!is_array($goods_id) || empty($goods_id)){
			$data['status'] = 2;	
			$data['msg'] = '请选择要确认支付的商品！';
			$this->ajaxReturn($data);
			exit();
		}
		if(empty($_SESSION['app']['uid'])){
			$data['status'] = 1;	
			$data['msg'] = '请登录之后在来提交订单！';
			$this->ajaxReturn($data);
			exit();
		}
		...
		foreach($goods_id as $v){
			if(!is_numeric($v))  {
				...
				$data_goods_number = M('cart')->where(" id=".$v.' and goods_type != 1' )->getField('goods_number');
				if( $data_goods_number && $data_goods_number<0.1){
					$data['status'] = 2;
					$data['msg'] = '确认订单中的商品数量不能为0';
					$this->ajaxReturn($data);
					exit();
				}
			}
	...
	**/
	public function  Confirm_Cart(){
		...
		$data_address=array();
		$user_address = M('user_address')->where("uid='".$_SESSION['app']['uid']."' AND country_id != 0 AND title !=''"."and is_default = 1".' and agent_id = '.C('agent_id'))->find();	
		// print_r($user_address);exit();
		if(!$user_address){
			$user_address = M('user_address')->where("uid='".$_SESSION['app']['uid']."' AND country_id != 0 AND title !=''".' and agent_id = '.C('agent_id'))->find();	
		}
	...
	**/
	public function Submit_Corder(){
		...
		$id = implode(',',$id);
	    $dataList = M('cart')->where("agent_id=".C('agent_id')." AND uid=".$_SESSION['app']['uid']." and id in ($id)")->order("goods_type ASC")->select(); 
		$getParentIDByUID = M('user')->where("uid='".$_SESSION['app']['uid']."' and agent_id = ".C('agent_id'))->getField('parent_id');
		...
		foreach($dataList as $key=>$val){ 
			...
			if($val['goods_type'] ==1 ){      // 1为裸钻
				...
				if(C('price_display_type') != 1) {
					$dataList[$key]['data']['dia_discount'] = ($dataList[$key]['data']['dia_discount'] +  $luozuan_advantage + $userdiscount);
					...
					$dataList[$key]['data']['dia_discount'] = $dataList[$key]['data']['dia_discount'] * (1+($luozuan_advantage  + $userdiscount)/100);
					$dataList[$key]['data']['price']        = formatRound( $dataList[$key]['data']['dia_global_price'] * C('dollar_huilv') * $dataList[$key]['data']['dia_discount'] * $dataList[$key]['data']['weight']/100, 2);  //国际报价*汇率*折扣*重量/100
				}
				...
				$dataList[$key]['data']                   = getGoodsListPrice($dataList[$key]['data'],$_SESSION['app']['uid'],'luozuan', 'single');
				...
				if($order_goods[$key]['attribute']['goods_price2'] != $order_goods[$key]['goods_price2']){
					...
					$action = M('cart') -> where(array('agent_id'=>C('agent_id'),'uid'=>$_SESSION['app']['uid'],'goods_id'=>$val['goods_id']))->save($save);
				}
		...
		if($this->createOrder($result)){
			...
			$data['msg'] = "订单提交成功";
			//发送用户消息
			$username = M("User")->where('uid='.$_SESSION['app']['uid'])->getField('username');
			...
			$s->sendMsg($_SESSION['app']['uid'],$content, $title);
		}else{
			$data['status'] = 0;
			$data['msg'] = "请勿提交相同的产品";
		}
	...
	**/
	public function Corder_succeed(){
		...
		$this->order_total=$_SESSION['Corder_succeed_total'];
		$this->display();	
	}
