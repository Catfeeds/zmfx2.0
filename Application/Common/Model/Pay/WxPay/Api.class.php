...
class Api
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		if(!$inputObj->IsNotify_urlSet()){
			$inputObj->SetNotify_url(C('WXPAY_NOTIFY_URL'));//异步通知url
		}
		
		$inputObj->SetAppid(C('WXPAY_APPID'));//公众账号ID
		$inputObj->SetMch_id(C('WXPAY_MCHID'));//商户号
	...
	{
		//如果不需要上报数据
		if(C('WXPAY_REPORT_LEVENL') == 0){
			return;
		} 
		//如果仅失败上报
		if(C('WXPAY_REPORT_LEVENL') == 1 &&
	...
	{		
		...
		//如果有配置代理这里就设置代理
		if(C('WXPAY_CURL_PROXY_HOST') != "0.0.0.0" && C('WXPAY_CURL_PROXY_PORT')  != 0){
			curl_setopt($ch,CURLOPT_PROXY, C('WXPAY_CURL_PROXY_HOST') );
			curl_setopt($ch,CURLOPT_PROXYPORT, C('WXPAY_CURL_PROXY_PORT') );
		}
