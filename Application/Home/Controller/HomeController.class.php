...
class HomeController extends Controller{
	...
	private function checkAgent(){
		...
		if($agent_id){
			...
			//redirect("http://www.btbzm.com/");exit;
			header("HTTP/1.0 404 Not Found");exit;	//备案需要设置非正常域名不能返回正确状态码
			//$this->error('非法域名!');
		}
	}
    ...
    private function setTemplate(){
        ...
        /*$templateInfo['template_path'] = 'default';
        $templateInfo['is_new'] = 0;*/
    	//组装文件地址
    	$color = strtolower($templateInfo['color']);
    	$file  = './Application/Home/View/'.$templateInfo['template_path'].'/Styles/Css/color/'.$color.'_'.C('agent_id').'.css';
    	if(file_exists($file)){
            $this -> getCssColorFile = substr($file, 1);
        }else {
            $this -> getCssColorFile = substr('./Application/Home/View/'.$templateInfo['template_path'].'/Styles/Css/color/'.$color.'.css', 1);
        }
        ...
        if(C('new_rules')['zgoods_show']){
            $this->zgoods_show = 1;
            $zgoods_show_arr = array(
                'index'=>array('index'),
                'index'=>array('index'),
                'goods'=>array('goodsinfo','goodscat'),
                'order'=>array('ordercart','orderconfirm','ordercomplete'),
                'user'=>array('logininfo','updatepwd','userinfo','orderlist','orderinfo'),
            );
            $low_controller_name = strtolower(CONTROLLER_NAME);
            $low_action_name = strtolower(ACTION_NAME);
            //未登录  && 需要验证  &&  不在不需要验证的地方里面
            if(isset($zgoods_show_arr[$low_controller_name]) && in_array($low_action_name,$zgoods_show_arr[$low_controller_name])){
                //用户区分新旧模板css,jss顺序
                $this->zgoods_show_type = 1;
            }
            //用于区分头部跟尾部的html
            $this->dingzhi_type = 1;
            $tmplParseString['__CSS_DZ__'] = __ROOT__.'/Application/Home/View/'.$templateInfo['template_path'].'/dingzhi/zhouliufu'.'/Styles/css';
            $tmplParseString['__IMG_DZ__'] = __ROOT__.'/Application/Home/View/'.$templateInfo['template_path'].'/dingzhi/zhouliufu'.'/Styles/img';
            $tmplParseString['__JS_DZ__'] = __ROOT__.'/Application/Home/View/'.$templateInfo['template_path'].'/dingzhi/zhouliufu'.'/Styles/js';
        }
        //周六福官网需求结束
        if( $templateInfo['is_new'] ){
            $controller_name = CONTROLLER_NAME;
            $action_name     = ACTION_NAME;
            $obj             = \Home\Controller\CtrFactoryController::createCtr($controller_name);
            if( $obj !== false ){
