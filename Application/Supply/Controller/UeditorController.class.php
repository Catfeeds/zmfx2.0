...
class UeditorController extends Controller
{
    ...
    public function _initialize(){
        ob_end_clean();
        $this->rootPath = './Public/Uploads/article/';
        $this->savePath = '';
        $this->thumb = 1;
    }
    ...
    public function index(){
        ...
        switch ($action) {
            ...
            default:
                $result = json_encode(array('state' => L('error_msg_082')));
                break;
        }
        ...
        if (isset($_GET["callback"])) {
            if (preg_match("/^[\w_]+$/", $_GET["callback"])) {
                ...
                echo json_encode(array('state' => L('error_msg_083')));
            }
        } else {
            echo $result;
        }
    ...
    private function upFile($config, $fieldName){
        $conf = array(
            ...
            'autoSub' => true,
            'subName' => date('Ym', time()), // 子目录命名的规则为 用户名/年月/
            'maxSize' => $config['maxSize'],
            'exts' => $this->format_exts($config['allowFiles'])
        );
        ...
        if ($info) {
            ...
            if ($isimage) {
                ...
                $image->Open($fname);
                //(张超豪修改2015-11-11)，改变上传图片的大小,2016-4-23修改为最大上传宽高为1920*1080
                //$image->thumb(680, 680, $this->thumb)->save($fname);
                $image->thumb(1920, 1080, $this->thumb)->save($fname);
                $width = $image->width() ;
            }
            ...
            $data = array(
                ...
                'url' => __ROOT__ . (substr($fname, 1)),//部让他主动改成小写的名字
                'title' => $info['savename'],
                'original' => $info['name'],
                'type' => '.' . $info['ext'],
                'size' => $info['size'],
                //(张超豪修改2015-11-11) 给图片加上宽度
                'width' => ( $width > 820)? '100%': '' ,
                'height' => 'auto',
            );
    ...
    private function upBase64($config, $fieldName){
        ...
        if ($file['filesize'] >= ($config["maxSize"])) {
            $data = array('state' => L('error_msg_084'));
            return json_encode($data);
        }

        // 创建目录失败
        if (! file_exists($dirname) && ! mkdir($dirname, 0777, true)) {
            $data = array('state' => L('error_msg_085'));
            return json_encode($data);
        } else{
            if (! is_writeable($dirname)) {
                $data = array('state' => L('error_msg_086'));
                return json_encode($data);
            }
        }
        // 移动文件
        if (! (file_put_contents($fullName, $img) && file_exists($fullName))) { // 移动失败
            $data = array('state' => L('error_msg_087'));
        } else { // 移动成功
            $data = array(
                'state' => 'SUCCESS',
