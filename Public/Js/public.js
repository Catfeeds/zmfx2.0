 ...
 *	②goods_id:	商品ID
*/
function diyStep(step,goods_id){
	if(step==1){setCookie('step1',goods_id);}
	if(step==2){setCookie('step2',goods_id);}  
	if(step == 2){   
		setCookie('goods_id_'+goods_id,$(".oGoods .active").attr('ref'));
	}
...
// 添加成品或戒托到购物车
function addToCart(type,goods_id){
	...
	$.post("/Home/Order/add2cart",{Action:'post',goodsType:type,gid:goods_id,specification_id:specification_id},function(data){
		alert(data.msg); 
		if(type == 2){
