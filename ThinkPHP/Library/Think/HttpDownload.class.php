...
class HttpDownload {
	...
	function __construct(){
		set_time_limit(0);ignore_user_abort(true);ini_set('memory_limit','1800M');//设置文件永不超时
	}
	
	function __destruct(){
	  error_log($this->m_error);
	}
	...
	*/
	public function SaveToBin($savefilename) {
		...
		while (!feof($this->m_fp)) {
			file_put_contents($savefilename.'.log', '正在下载文件：'.filesize($savefilename).'B',FILE_APPEND);
	...
	*/
	public function GetHtml() {
		...
		while(!feof($this->m_fp)) {
			file_put_contents('HttpDownload.log', '正在下载文件：'.strlen($this->m_html)."B\n",FILE_APPEND);
	...
	*/
	...
	public function downloadA2B($httpfileUrl,$localfileName){
		if(empty($httpfileUrl) or empty($localfileName)){
			return false;
		}else{
			$this->OpenUrl($httpfileUrl);	/*打开远程地址*/
			$this->SaveToBin($localfileName);	/*保存远程文件*/
			$this->Close();		/*关闭释放资源*/
			return true;
		}
	}
	
	public function viewWebpage($url){
		if(empty($url)){
			return false;
		}else{
			$this->OpenUrl($url);
...
$a = 'http://www.gia.edu/otmm_wcs_int/proxy-pdf/?ReportNumber=6137981943&url=https://myapps.gia.edu/ReportCheckPOC/pocservlet?ReportNumber=6137981943';
$b = '6137981943.pdf';
$file->downloadA2B($a,$b);
*/
?>
