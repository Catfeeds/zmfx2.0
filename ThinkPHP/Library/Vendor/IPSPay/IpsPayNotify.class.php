                ...
                if($strRspCode == "000000"){
                    ...
                    if($strStatus == "Y"){
                        ...
                        if(md5Verify($strBody,$strSignature,$this->ipspay_config["MerCode"],$this->ipspay_config["MerCert"])){                            
                            $result['status']=true;
                            $result['data']['merBillNo']=$xmlResult->GateWayRsp->body->MerBillNo;
                            $result['data']['amount']=$xmlResult->GateWayRsp->body->Amount;
                            $result['data']['ipsBillNo']=$xmlResult->GateWayRsp->body->IpsBillNo;
                            $result['data']['ipsTradeNo']=$xmlResult->GateWayRsp->body->IpsTradeNo;  
                            return $result;
                        }else{
